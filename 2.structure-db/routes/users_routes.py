...
async def siginin_user(data : UserSignIn=Form(...), session = Depends(get_session)) -> dict:
    # email, pwd 비교
    ...
    for user in users:
        if data.email == user.email:
            if hash_password.verify_hash(data.password, user.password): #data.password == user.password:
